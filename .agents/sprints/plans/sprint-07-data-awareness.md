...
# Sprint 7 — Data awareness (Bronze profiling + PII auto-discovery)
...
scope for Sprint 7**. This TD is a carryover from Sprint 6 and requires a live Fabric workspace.
Sprint 7 features (profiling, PII) are new Python notebooks with no dependency on the live Fabric
SQL commissioning path. The SQLite stub fully covers offline test coverage for control table reads.

No other High TDs are targeted at Sprint 7.
...
- [ ] `pytest tests/test_nb_bronze_profiling.py -v` — all green
- [ ] `pytest tests/test_nb_pii_scanner.py -v` — all green
- [ ] `pytest tests/test_nb_catalog_sync.py -k "PiiAutoFlag" -v` — all green
- [ ] `pytest tests/test_nb_landing_to_bronze.py -k "BronzeProfilingIntegration" -v` — all green
...
- [ ] ADR-048 committed before first GAP-07 implementation commit
- [ ] ADR-049 committed before first GAP-08 implementation commit
- [ ] `profiling.ColumnStats` has `DistributionHistogram` column in `nb_bootstrap.py` DDL
- [ ] `profiling.PiiCandidates` DDL in `nb_bootstrap.py`
- [ ] `BronzeProfilingDriftThreshold` row in `config/seed-dev.json`
- [ ] `PiiScanConfidenceThreshold` row in `config/seed-dev.json`
