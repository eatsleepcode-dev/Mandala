...
- [x] `pytest tests/test_nb_data_quality_contracts.py -v` — all green (22/22 ✅)
- [x] `pytest tests/test_nb_bronze_to_silver.py -k "ContractValidationIntegration" -v` — all green (3/3 ✅ — class `TestValidateWithContractsIntegration`)
- [x] `pytest tests/ --cov=src/notebooks --cov-fail-under=80` — ≥80% (995 passing; 8 pre-existing failures only)
- [x] `gitleaks detect --verbose` — clean (pre-existing baseline)
...
- [x] ADR-050 committed before first implementation commit (`docs/adr/ADR-050-dq-framework-pandera-vs-ge-vs-pydeequ.md` — Accepted ✅)
- [x] `control.TableContracts` DDL in `nb_bootstrap.py` includes `IsEnabled` column (line 452 ✅)
- [x] `_validate_with_contracts` extracted from `process_object` in `nb_bronze_to_silver.py` (lines 467–482 ✅)
- [x] `CheckParam=NULL` guard confirmed by `TestParseCheckParam::test_none_returns_none` ✅
- [x] Quarantine-write failure graceful handling confirmed by `TestValidateDataframe::test_quarantine_write_error_is_non_fatal` ✅
